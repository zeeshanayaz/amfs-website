...
import { isValidApplication, isValidEmail, submitJobApplication } from '@/lib/supabase/client'

type CareersPageProps = { jobs: JobPost[] }
      ...
      <div className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-background p-6 shadow-2xl sm:max-w-2xl sm:rounded-3xl sm:p-8">
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-semibold text-brand-navy">Full name<input required value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="rounded-xl border border-brand-border bg-brand-off-white px-4 py-3 font-normal outline-none focus:border-brand-royal" /></label>
            <label className="grid gap-2 text-sm font-semibold text-brand-navy">Email address<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="rounded-xl border border-brand-border bg-brand-off-white px-4 py-3 font-normal outline-none focus:border-brand-royal" /></label>
            <label className="grid gap-2 text-sm font-semibold text-brand-navy sm:col-span-2">Phone number<input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="rounded-xl border border-brand-border bg-brand-off-white px-4 py-3 font-normal outline-none focus:border-brand-royal" /></label>
            <label className="grid gap-2 text-sm font-semibold text-brand-navy sm:col-span-2">Why would you be a great fit?<textarea required minLength={20} rows={5} value={form.cover_letter} onChange={(e) => setForm({ ...form, cover_letter: e.target.value })} className="resize-y rounded-xl border border-brand-border bg-brand-off-white px-4 py-3 font-normal outline-none focus:border-brand-royal" /></label>
    ...
    <>
      ...
      <section className="bg-brand-off-white px-6 py-16 sm:px-10 lg:px-16 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          {jobs.length === 0 ? <div className="rounded-3xl border border-dashed border-brand-border bg-background p-12 text-center"><h3 className="font-serif text-2xl font-bold text-brand-navy">No open roles right now</h3><p className="mx-auto mt-3 max-w-md text-brand-dark-gray">We are always happy to hear from thoughtful educators. Please check back soon for new opportunities.</p></div> : <div className="grid gap-4">{jobs.map((job) => <article key={job.id} className="group flex flex-col justify-between gap-6 rounded-3xl border border-brand-border bg-background p-6 transition hover:-translate-y-1 hover:border-brand-sky hover:shadow-lg sm:flex-row sm:items-center sm:p-8"><div><div className="mb-3 flex flex-wrap gap-2 text-xs font-bold uppercase tracking-wider"><span className="rounded-full bg-brand-light px-3 py-1 text-brand-royal">{job.department}</span><span className="rounded-full bg-brand-gold/30 px-3 py-1 text-brand-navy">{job.employment_type}</span></div><h3 className="font-serif text-2xl font-bold text-brand-navy">{job.title}</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-brand-dark-gray">{job.description}</p><p className="mt-4 flex items-center gap-2 text-sm font-semibold text-brand-royal"><MapPin className="size-4" />{job.location}</p></div><button type="button" onClick={() => setSelectedJob(job)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-brand-navy px-5 py-3 text-sm font-bold text-primary-foreground transition hover:bg-brand-royal">View role <ArrowRight className="size-4 transition group-hover:translate-x-1" /></button></article>)}</div>}
        </div>
      </section>
