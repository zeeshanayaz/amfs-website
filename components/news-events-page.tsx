      ...
      <main className="bg-brand-off-white text-brand-navy">
          ...
          <section ref={detailRef} className="mx-auto max-w-7xl px-6 py-12 sm:px-10">
            <article className="overflow-hidden rounded-[28px] border border-brand-border bg-white shadow-sm">
              ...
              <div className="p-8 sm:p-10">
                ...
                <h2 className="mt-5 font-serif text-4xl font-bold text-brand-navy sm:text-5xl">{selectedItem.title}</h2>
                <p className="mt-5 max-w-3xl text-lg leading-8 text-brand-dark-gray">{selectedItem.excerpt}</p>
                ...
                <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <p className="text-xs uppercase tracking-[0.25em] text-brand-dark-gray">Scroll to continue reading other stories below</p>
                </div>
              </div>
            </article>
          </section>
        ...
        <section className="mx-auto max-w-7xl px-6 py-16 sm:px-10">
          {loading ? (
            ...
            <NewsEventsList items={items} onItemSelect={setSelectedItem} onImagePreview={setPreviewImage} />
          )}
        </section>
      </main>
      <Lightbox open={Boolean(previewImage)} title={previewImage?.title} onClose={() => setPreviewImage(null)}>
        {previewImage ? (
          <Image
